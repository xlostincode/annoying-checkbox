...
const isInsideRect = (x: number, y: number, rw: number, rh: number) => {
...
const Checkbox = <Value,>(props: Props<Value>) => {
      ...
      while (queue.length) {
        ...
        for (const [offsetX, offsetY] of directions) {
          ...
          if (
            ...
            isInsideRect(nextX, nextY, props.width, props.height)
          ) {
  ...
  const sketch = React.useCallback(
    (p5: P5CanvasInstance) => {
      ...
      p5.draw = () => {
        ...
        if (p5.mouseIsPressed) {
          if (isInsideRect(p5.mouseX, p5.mouseY, props.width, props.height)) {
    ...
    [props.width, props.height, validateFill]
  );
  ...
  return (
    <div className="flex gap-2">
      <div
        role="radio"
        className="flex items-center justify-center gap-2 cursor-pencil"
      >
